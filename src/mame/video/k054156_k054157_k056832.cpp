...
template<bool gflipx, bool gflipy> void k054156_056832_device::draw_page_512x1(bitmap_ind16 *bitmap, int layer, const rectangle &cliprect, const uint32_t *page, gfx_element *g, uint32_t min_x, uint32_t max_x, uint32_t min_y, uint32_t max_y, int32_t basex, int32_t basey)
{
	...
	for(uint32_t y = min_y; y <= max_y; y++) {
		uint32_t info = page[(y ^ (gflipy ? 255 : 0))];
		...
		g->opaque(*bitmap, cliprect, code, color, flipx, flipy, basex, basey + y);
	}
}
...
template<bool gflipx, bool gflipy> void k054156_056832_device::draw_page_8x8(bitmap_ind16 *bitmap, int layer, const rectangle &cliprect, const uint32_t *page, gfx_element *g, uint32_t min_x, uint32_t max_x, uint32_t min_y, uint32_t max_y, int32_t basex, int32_t basey)
{
	...
	for(uint32_t y = tile_min_y; y <= tile_max_y; y++) {
		const uint32_t *tiles = page + ((y ^ (gflipy ? 31 : 0)) << 6) + (tile_min_x ^ (gflipx ? 63 : 0));
		for(uint32_t x = tile_min_x; x <= tile_max_x; x++) {
			...
			g->opaque(*bitmap, cliprect, code, color, flipx, flipy, basex + (x << 3), basey + (y << 3));
		}
	}
}

template<bool gflipx, bool gflipy> void k054156_056832_device::draw_line_block(bitmap_ind16 *bitmap, int layer, uint32_t min_y, uint32_t max_y, uint32_t min_x, uint32_t max_x, uint32_t deltay, uint32_t deltax)
{
	rectangle cliprect(min_x, max_x, min_y, max_y);
	uint32_t base_min_x = min_x + deltax;
	uint32_t base_max_x = max_x + deltax;
	uint32_t base_min_y = min_y + deltay;
	uint32_t base_max_y = max_y + deltay;
	uint32_t pxmin = (base_min_x >> 9) & 7;
	uint32_t pxmax = (base_max_x >> 9) & 7;
	uint32_t pymin = (base_min_y >> 8) & 7;
	uint32_t pymax = (base_max_y >> 8) & 7;
	...
	for(;;) {
		...
		int32_t basey = (- deltay + (py << 8)) & 0x7ff;
		if(basey & 0x400)
			basey -= 0x800;
		...
		for(;;) {	
			...
			int32_t basex = (- deltax + (px << 9)) & 0xfff;
			if(basex & 0x800)
				basex -= 0x1000;
...
void k054156_056832_device::bitmap_update(bitmap_ind16 *bitmap, const rectangle &cliprect, int layer)
{
	...
	draw_line_block<false, false>(bitmap, layer, cliprect.min_y, cliprect.max_y, cliprect.min_x, cliprect.max_x, m_mv[layer] + 1, m_mh[layer] - xdelta[layer]);
}
